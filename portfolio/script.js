...
window.addEventListener('DOMContentLoaded', () => {
    const animatedElements = document.querySelectorAll(
        '.about-content, .skills-category, .project-card, .resume-content, .contact-content'
    );
...
function viewResume() {
    ...
    setTimeout(() => {
        window.open('#', '_blank'); // In real app, this would be the actual PDF URL
    }, 1000);
}
...
window.addEventListener('scroll', () => {
    ...
    if (profilePicture && window.innerWidth > 768) {
        ...
        profilePicture.style.transform = `translateY(${speed}px)`;
    }
});
...
// Add the ripple CSS to the existing style
const existingStyle = document.querySelector('style');
if (existingStyle) {
    existingStyle.textContent += rippleCSS;
} else {
    const newStyle = document.createElement('style');
    newStyle.textContent = rippleCSS;
